...
/* Hero mockup: our own clean, CSS-drawn iPhone frame (ink bezel, rounded
   corners, dynamic-island pill — not Apple device artwork) framing the REAL,
   full ONIS app screen. Driven by `progress` (0–1) from the sticky scroll
   container: a +1 logs the energy-drinks count, then it crossfades to Coach. */
...
function HomeScreen({ countP, btnScale }: { countP: number; btnScale: number }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: colors.canvas, padding: "6cqw 5.5cqw 0", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "3.6cqw" }}>
        <Card icon={<Bolt c={APP.energy} />} iconBg={APP.energySoft} title="Energy drinks" period="Today" count={countRoll} target={2} below={logged ? 1 : 2} accent={APP.energy} barFrac={countP * 0.5} btnScale={btnScale} dotFill={ss(countP, 0.45, 1)} />
...
export default function HeroComposite({ progress, reduce }: { progress: number; reduce: boolean }) {
  // Scroll scrub: the +1 button depresses, the energy-drinks count rolls up,
  // then the screen crossfades to the Coach plan.
  const press = reduce ? 0 : ss(progress, 0.36, 0.4) - ss(progress, 0.46, 0.54);
  const countP = reduce ? 0 : ss(progress, 0.4, 0.54);
  const btnScale = 1 - 0.08 * press;
  const coachOpacity = reduce ? 0 : ss(progress, 0.66, 0.82);
  const todayOpacity = 1 - coachOpacity;

  return (
    <div className="hero-phone" role="img" aria-label="The ONIS app running on a phone, logging an energy-drink count">
      <div className="hero-screen">
        <div style={{ position: "absolute", inset: 0, opacity: todayOpacity }}>
          <HomeScreen countP={countP} btnScale={btnScale} />
        </div>
